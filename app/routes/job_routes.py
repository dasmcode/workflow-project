...
def get_all_jobs(db:Session = Depends(get_db)):
    try:
        ...
        job_list = [{"job_id": str(job.id), "file_id": str(job.file_id), "workflow_type": job.workflow_type, "status": job.status.value, "current_step": job.current_step} for job in jobs]
...
def delete_job(job_id:str, db:Session = Depends(get_db)):
    try:
        ...
        db.delete(job)
        db.commit()
        return JSONResponse(content={"message": f"Job with ID {job_id} deleted successfully"}, status_code=200)
    except Exception as e:
        db.rollback()
        return JSONResponse(content={"error": str(e)}, status_code=500)
...
def delete_all_jobs(db:Session = Depends(get_db)):
    try:
        ...
        db.commit()
        return JSONResponse(content={"message": f"All jobs deleted successfully. Total deleted: {num_deleted}"}, status_code=200)
    except Exception as e:
        db.rollback()
        return JSONResponse(content={"error": str(e)}, status_code=500)